...
def _load_required(path: Path):
    if not path.exists():
        raise FileNotFoundError(
            f"Missing model artifact: {path}. Run train_models.py locally first."
        )
...
try:
    logistic_model = _load_required(MODEL_DIR / "logistic_regression.joblib")
    random_forest = _load_required(MODEL_DIR / "random_forest.joblib")
    meta_path = MODEL_DIR / "metadata.json"
    if meta_path.exists():
        metadata = json.loads(meta_path.read_text(encoding="utf-8"))
except FileNotFoundError:
    # Keep the module importable; endpoint returns a helpful 500 until artifacts exist.
    pass
...
def health():
    return {
        "status": "ok",
        "modelsLoaded": logistic_model is not None and random_forest is not None,
    }
...
def predict(patient: PatientInput):
    if logistic_model is None or random_forest is None:
        raise HTTPException(
            status_code=500,
